...
void main() {
  group('buildPosTheme', () {
    test('attaches the PosStatusColors extension (light + dark)', () {
      expect(
        buildPosTheme().extension<PosStatusColors>(),
    ...
    test(
      'keeps the deepOrange-derived terracotta primary (refine-not-reskin)',
      () {
  ...
  test('PosStatusColors lerp is stable at the endpoints', () {
    expect(
      PosStatusColors.light.lerp(PosStatusColors.dark, 0).success,
      PosStatusColors.light.success,
    );
    expect(
      PosStatusColors.light.lerp(PosStatusColors.dark, 1).success,
